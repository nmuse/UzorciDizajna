...
namespace nmuse_zadaca_2.FactoryMethod
{
    class UcitajIgrace : IProduct
    {
        public void ucitavanjeDatoteke(string datotekaZaUcitavanje)
        {
            ...
            {
                Console.WriteLine("Neispravna datoteka igraca");
                Environment.Exit(1);
            }
            ...
            foreach (String linija in linije)
            {
                ...
                Igrac noviIgrac = new Igrac(podaci[0], imePrezime, listaPozicija, dt);
                List<Klub> klubovi = p1.ListaKlubova;
                foreach (Klub k in klubovi) {
                    if (k.Kratica.Equals(klub.Kratica))
                    {
                        k.dodaj(noviIgrac);
                    }
                }
